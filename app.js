...
function renderDiets(){const d=[...S.DIETS].sort((a,b)=>String(b.startDate).localeCompare(String(a.startDate)));$('#dietCards').innerHTML=d.length?d.map(x=>`<article class="timelineCard"><header><div><span class="badge">${x.endDate?'Concluso':'In corso'}</span><h3>${esc(x.name)}</h3></div>${actions(x.id,'DIETS')}</header><p>${dateFmt(x.startDate)} → ${x.endDate?dateFmt(x.endDate):'oggi'}</p><p class="muted">${x.kcal?esc(x.kcal)+' kcal/giorno · ':''}${x.carbsG?esc(x.carbsG)+' g carboidrati · ':''}${x.weightStart?esc(x.weightStart)+' kg iniziali':''}</p>${x.notes?`<p>${esc(x.notes)}</p>`:''}</article>`).join(''):'<p class="muted">Nessun periodo inserito.</p>'}
